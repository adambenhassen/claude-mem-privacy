...
describe('SettingsDefaultsManager', () => {
  ...
  beforeEach(() => {
    ...
    // The preload tripwire (tests/preload.ts) pins CLAUDE_MEM_DATA_DIR for the
    // whole run, and loadFromFile applies env overrides on top of file values —
    // which would make every loadFromFile result diverge from getAllDefaults()'s
    // hardcoded defaults. These tests are about file > defaults behavior on an
    // EXPLICIT settingsPath (no real data-dir I/O happens here), so drop that env
    // override for their duration and restore it after.
