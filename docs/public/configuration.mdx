...
| `CLAUDE_MEM_ALLOWED_PROJECTS` | _(empty)_                       | Comma-separated glob patterns; when set, only matching project paths are tracked |
| `CLAUDE_MEM_EXCLUDED_PROJECTS` | _(empty)_                      | Comma-separated glob patterns; matching project paths are never tracked (overrides the allow-list) |
