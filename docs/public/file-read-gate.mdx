...
By default claude-mem tracks every project. Two settings narrow this:

- `CLAUDE_MEM_ALLOWED_PROJECTS` — when non-empty, **only** project paths matching one of its patterns are tracked.
- `CLAUDE_MEM_EXCLUDED_PROJECTS` — project paths matching one of its patterns are never tracked. Exclusions take precedence over the allow-list.

Both accept comma-separated glob patterns (`*`, `**`, `?`, and a leading `~`). Configure them in `~/.claude-mem/settings.json` or via the viewer's Settings → Project Filtering panel:

```json
{
  "CLAUDE_MEM_ALLOWED_PROJECTS": "~/work/*,~/clients/**",
